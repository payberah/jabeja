...
public class JabejaNode implements CDProtocol {
	...
	private static double LOCALITY = 1000;
...
//---------------------------------------------------------------------
	public JabejaNode(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNode partitionNode = null;
		
		try { 
			partitionNode = (JabejaNode)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNode)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNode bestNode = null;
		double highestBenefit = 0;
		Node next = null;
		JabejaNode selfNode = (JabejaNode)n.getProtocol(protocolId);
		ArrayList<JabejaNode> randomNodes = new ArrayList<JabejaNode>();
		...
		for (int j = 0; j < NUM_OF_WALKS; j++) {
			JabejaNode randomNode = selfNode;
			...
			for (int i = 0; i < this.walkLength; i++) {
				...
				randomNode = (JabejaNode)next.getProtocol(protocolId);
			}
		...
		for (JabejaNode tempNode : randomNodes) {
			...
			if (this.initialColor == tempNode.initialColor) {
				newBenefit *= LOCALITY;
				oldBenefit *= LOCALITY;
			}
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		...
		while(true) {
			...
			partitionNode = (JabejaNode)next.getProtocol(this.protocolId);
