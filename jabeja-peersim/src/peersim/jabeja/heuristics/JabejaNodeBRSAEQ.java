...
public class JabejaNodeBRSAEQ implements CDProtocol {
	...
	public float noise = (float)2;
	private float noiseDelta = (float)0.003;

//---------------------------------------------------------------------
	public JabejaNodeBRSAEQ(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNodeBRSAEQ partitionNode = null;
		
		try { 
			partitionNode = (JabejaNodeBRSAEQ)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		JabejaNodeBRSAEQ neighbourNode;
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNodeBRSAEQ)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNodeBRSAEQ bestNode = null;
		double highestBenefit = 0;
		Node next = null;
		JabejaNodeBRSAEQ selfNode = (JabejaNodeBRSAEQ)n.getProtocol(protocolId);
		ArrayList<JabejaNodeBRSAEQ> randomNodes = new ArrayList<JabejaNodeBRSAEQ>();
		...
		for (int j = 0; j < NUM_OF_WALKS; j++) {
			JabejaNodeBRSAEQ randomNode = selfNode;
			...
			for (int i = 0; i < this.walkLength; i++) {
				...
				randomNode = (JabejaNodeBRSAEQ)next.getProtocol(protocolId);
			}
		...
		for (JabejaNodeBRSAEQ tempNode : randomNodes) {
		...
		if (bestNode == null) {
			...
			for (Node node : newscastNodes) {
				if (node != null)
					randomNodes.add((JabejaNodeBRSAEQ)node.getProtocol(protocolId));
			}
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		Node next = null;
		JabejaNodeBRSAEQ partitionNode;
		...
		while(true) {
			...
			partitionNode = (JabejaNodeBRSAEQ)next.getProtocol(this.protocolId);
