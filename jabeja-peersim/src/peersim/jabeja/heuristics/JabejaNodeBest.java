...
public class JabejaNodeBest implements CDProtocol {
...
//---------------------------------------------------------------------
	public JabejaNodeBest(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNodeBest partitionNode = null;
		
		try { 
			partitionNode = (JabejaNodeBest)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNodeBest)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNodeBest bestNode = null;
		double highestBenefit = 0;
		Node next = null;
		JabejaNodeBest selfNode = (JabejaNodeBest)n.getProtocol(protocolId);
		ArrayList<JabejaNodeBest> randomNodes = new ArrayList<JabejaNodeBest>();
		...
		for (int j = 0; j < NUM_OF_WALKS; j++) {
			JabejaNodeBest randomNode = selfNode;
			...
			for (int i = 0; i < this.walkLength; i++) {
				...
				randomNode = (JabejaNodeBest)next.getProtocol(protocolId);
			}
		...
		for (JabejaNodeBest tempNode : randomNodes) {
		...
		if (bestNode == null) {
			...
			for (Node node : newscastNodes) {
				if (node != null)
					randomNodes.add((JabejaNodeBest)node.getProtocol(protocolId));
			}
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		...
		while(true) {
			...
			partitionNode = (JabejaNodeBest)next.getProtocol(this.protocolId);
