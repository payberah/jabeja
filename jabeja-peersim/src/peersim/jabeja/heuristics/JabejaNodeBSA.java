...
public class JabejaNodeBSA implements CDProtocol {
...
//---------------------------------------------------------------------
	public JabejaNodeBSA(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNodeBSA partitionNode = null;
		
		try { 
			partitionNode = (JabejaNodeBSA)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNodeBSA)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNodeBSA bestNode = null;
		double highestBenefit = 0;
		Node next = null;
		JabejaNodeBSA selfNode = (JabejaNodeBSA)n.getProtocol(protocolId);
		ArrayList<JabejaNodeBSA> randomNodes = new ArrayList<JabejaNodeBSA>();
		...
		for (int j = 0; j < NUM_OF_WALKS; j++) {
			JabejaNodeBSA randomNode = selfNode;
			...
			for (int i = 0; i < this.walkLength; i++) {
				...
				randomNode = (JabejaNodeBSA)next.getProtocol(protocolId);
			}
		...
		for (JabejaNodeBSA tempNode : randomNodes) {
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		...
		while(true) {
			...
			partitionNode = (JabejaNodeBSA)next.getProtocol(this.protocolId);
