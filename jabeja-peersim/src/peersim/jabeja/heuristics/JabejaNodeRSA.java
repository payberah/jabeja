...
public class JabejaNodeRSA implements CDProtocol {
...
//---------------------------------------------------------------------
	public JabejaNodeRSA(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNodeRSA partitionNode = null;
		
		try { 
			partitionNode = (JabejaNodeRSA)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNodeRSA)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNodeRSA bestNode = null;
		double highestBenefit = 0;
		JabejaNodeRSA selfNode = (JabejaNodeRSA)n.getProtocol(protocolId);
		ArrayList<JabejaNodeRSA> randomNodes = new ArrayList<JabejaNodeRSA>();
		...
		if (bestNode == null) {
			...
			for (Node node : newscastNodes) {
				if (node != null)
					randomNodes.add((JabejaNodeRSA)node.getProtocol(protocolId));
			}

			for (JabejaNodeRSA tempNode : randomNodes) {
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		...
		while(true) {
			...
			partitionNode = (JabejaNodeRSA)next.getProtocol(this.protocolId);
