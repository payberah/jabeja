...
public class JabejaNodeBRSA implements CDProtocol {
...
//---------------------------------------------------------------------
	public JabejaNodeBRSA(String prefix) {
...
//---------------------------------------------------------------------
	public Object clone() {
		JabejaNodeBRSA partitionNode = null;
		
		try { 
			partitionNode = (JabejaNodeBRSA)super.clone(); 
...
//--------------------------------------------------------------------
	public HashMap<Integer, Integer> getNeighboursColors() {
		...
		for (Node node : neighbours) {
			neighbourNode = (JabejaNodeBRSA)node.getProtocol(this.protocolId);
...
//--------------------------------------------------------------------
	public void swap(Node n, int protocolId) {
		JabejaNodeBRSA bestNode = null;
		double highestBenefit = 0;
		Node next = null;
		JabejaNodeBRSA selfNode = (JabejaNodeBRSA)n.getProtocol(protocolId);
		ArrayList<JabejaNodeBRSA> randomNodes = new ArrayList<JabejaNodeBRSA>();
		...
		for (int j = 0; j < NUM_OF_WALKS; j++) {
			JabejaNodeBRSA randomNode = selfNode;
			...
			for (int i = 0; i < this.walkLength; i++) {
				...
				randomNode = (JabejaNodeBRSA)next.getProtocol(protocolId);
			}
		...
		for (JabejaNodeBRSA tempNode : randomNodes) {
		...
		if (bestNode == null) {
			...
			for (Node node : newscastNodes) {
				if (node != null)
					randomNodes.add((JabejaNodeBRSA)node.getProtocol(protocolId));
			}
...
//--------------------------------------------------------------------
	public Node getRandomNode() {
		...
		while(true) {
			...
			partitionNode = (JabejaNodeBRSA)next.getProtocol(this.protocolId);
